...
- (IBAction)topRight:(id)sender {
    ...
    DXPopover *popover = [DXPopover popover];
    [popover showAtView:sender withContentView:textLabel];
}
...
//Note: Here if you don't give the position down, then it will show on up, because the atView's distance from containerViews' top edge is larger than the distance from the containerView's bottom edge.
- (IBAction)middle:(id)sender {
    ...
    DXPopover *popover = [DXPopover popover];
    [popover showAtView:sender popoverPostion:DXPopoverPositionDown withContentView:textLabel inView:self.tabBarController.view];
}
